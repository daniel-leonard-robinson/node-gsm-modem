            ...
            this.getSMS(storage, smsId, function (err, msg) {
                if (err === undefined) {
                    ...
                    if (trackingObj && trackingObj.parts === 0) {
                        ...
                        reportObj.reports = reportObj.reports.sort(function (a, b) {
            ...
            this.getStorages(function (err, storages) {
                ...
                if (!err) {
                    for (i = 0; i < storages.outbox.length; ++i) {
                        if (storages.outbox[i] === '"ME"') {
                            supportOutboxME = true;
                            break;
                        }
                    }
                    for (i = 0; i < storages.inbox.length; ++i) {
                        if (storages.inbox[i] === '"ME"') {
                            supportInboxME = true;
                            break;
                        }
                    }
 ...
 */
Modem.prototype._setConnectionProfile = function (config, cb) {
    this.enableInternet(false, function (err) {
        if (err) cb('[enableInternet false] ' + err);
        else {
            this.sendCommand('AT+UPSD=0,1,\"' + config.apn + '\"', function (data) {
                    this.OK__cb_if_Err(data, cb)
                }.bind(this)
            );
            this.sendCommand('AT+UPSD=0,2,\"' + config.username + '\"', function (data) {
                    this.OK__cb_if_Err(data, cb)
                }.bind(this)
            );
            this.sendCommand('AT+UPSD=0,3,\"' + config.password + '\"', function (data) {
                    this.OK__cb_if_Err(data, cb)
                }.bind(this)
            );
            this.sendCommand('AT+UPSD=0,4,\"' + config.dns1 + '\"', function (data) {
                    this.OK__cb_if_Err(data, cb)
                }.bind(this)
            );
            this.sendCommand('AT+UPSD=0,5,\"' + config.dns2 + '\"', function (data) {
                    this.OK__cb_if_Err(data, cb)
                }.bind(this)
            );
        }
 ...
 * Checks if OK was received.
 * @param data  looks inside for 'OK'
 * @param cb  to be called upon error.
 */
Modem.prototype.OK__cb_if_Err = function (data, cb) {
    if (data.indexOf('OK') === -1) {
        cb(new Error(data))
    } else {
        cb(undefined)
...
/**
 * Compares two connection profiles
 * @param _cur current profile
 * @param _new new profile
 * @returns {boolean} true if the same
 * @private
 */
Modem.prototype._compareConnectionProfile = function(_cur, _new) {
    console.dir(_cur);
    intel.debug("wolol");
    console.dir(_new);
    intel.debug((_cur.apn !== _new.apn));
    intel.debug(_cur.username !== _new.username);
    intel.debug(_cur.dns1 !== _new.dns1);
    intel.debug(_cur.dns2 !== _new.dns2);
    intel.debug(_new.password !== '');

    if (_cur.apn !== _new.apn) return false;
    else if (_cur.username !== '') return false;
    else if (_cur.dns1 !== _new.dns1) return false;
    else if (_cur.dns2 !== _new.dns2) return false;
    else if (_new.password !== '') return false;
    return true;
}

Modem.prototype.setupInternet = function (config, cb) {
    if (config.apn === undefined) config.apn = '';
    if (config.username === undefined) config.username = '';
    if (config.password === undefined) config.password = '';
    if (config.dns1 === undefined) config.dns1 = '0.0.0.0';
    if (config.dns2 === undefined) config.dns2 = '0.0.0.0';

    this.getConnectionProfile(function (err, data) {
        if (!err) {
            if (!this._compareConnectionProfile(data, config)) {
                this._setConnectionProfile(config, function (err) {
                    if (err) cb('[_setConnectionProfile] ' + err);
                })
            }
            ;
            this.enableInternet(true, function (err) {
                if (err) cb('[enableInternet true] ' + err);
                else {
                    cb(undefined)
                }
            })
        } else cb('[setupInternet] ' + err);
    }.bind(this));
}
